...
SGameManager::SGameManager()
{
    ...
    m_buttonSelectionLargeSprite =
    std::make_shared<SSprite>("./assets/art/ui/buttonSelection.png", 1, 30, 5);
    ...
    m_buildingConstructionSprite =
    std::make_shared<SSprite>("./assets/art/buildingConstruction.png", 1, 60, 2);
    ...
    spawnUnitsForPlayer(spawnTile, "UNIT_SPEARMAN", defaultPlayer.getPlayerId(), 3);
...
void SGameManager::run()
{
    ...
    while (!m_bQuit)
    {
        ...
        if (m_maxFPS != 0)
        {
            ...
            std::max(0, static_cast<int>(1000000 * (m_maxFrameTime - m_deltaTime)))));
        }
...
void SGameManager::handleRendering()
{
    ...
    if (m_selectedTile != nullptr)
    {
        ...
        {
            auto constructableBuildings = getConstructableBuidlings(m_selectedTile, defaultPlayer);
            int dx = 5;
            int dy = 5;
    ...
    for (auto& p : m_units)
    {
        ...
        auto strongestUnit = std::max_element(unitSet.begin(), unitSet.end(), compareUnits);
    ...
    m_renderer.submitTextRenderRequest(std::to_string(defaultPlayer.getCurrentManaAmount()),
                                       m_virtualWidth - 35, 10, m_manaIconSprite->m_size, 5,
                                       RenderLocation::RENDER_TOP_RIGHT);
...
void SGameManager::performUnitMovement()
{
    ...
    for (const auto& [u, t] : m_unitTargetTiles)
    {
        units.insert(u);
    }
    performUnitMovement(units);
}
...
void SGameManager::handleLeftClick()
{
            ...
            if (m_selectedBuilding != nullptr and tu.size() > 0)
            {
                ...
                if (bClickedTrainingIcon)
                {
                    ...
                    if (index < tu.size())
                    {
                        ...
                        defaultPlayer.removeResources(m_unitLookUpTable[targetUnit].m_resourceCost);
                        return;
                    }
                }
            }
    ...
    if (m_selectedUnits.size() > 0 and y <= m_virtualHeight - 5 and y >= m_virtualHeight - 35)
    {
        ...
        if (bClickedBuildingIcon)
        {
            ...
            if (index < cb.size())
            {
                ...
                constructBuidlingForPlayer(m_selectedTile, targetBuilding, defaultPlayer);
                m_selectedUnits = {};
            }
            return;
        }
    ...
    auto clickedTile = getClickedTile(x, y);
    if (clickedTile == nullptr or (m_units[clickedTile].size() == 0 and m_buildings.count(clickedTile) == 0))
    {
        m_selectedUnits.clear();
        m_selectedBuilding = nullptr;
        return;
...
bool SGameManager::bCanConstructBuilding(std::shared_ptr<SNode> tile, std::string buildingId, SPlayer& player)
{
...
std::unordered_set<std::string> SGameManager::getConstructableBuidlings(std::shared_ptr<SNode> tile, SPlayer& player)
{
...
SGameManager::constructBuidlingForPlayer(std::shared_ptr<SNode> tile, BUILDING_ID building, SPlayer& player)
{
...
std::shared_ptr<SBuilding>
SGameManager::spawnBuidlingForPlayer(std::shared_ptr<SNode> tile, BUILDING_ID building, SPlayer& player)
{
...
{
    //  m_players[unit->getOwner()]->addResources(tile->getAndRemoveMana());
